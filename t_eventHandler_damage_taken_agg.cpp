...
t_eventHandler_damage_taken_agg::t_eventHandler_damage_taken_agg(MainWindow *mainWindow)
{
    this->mainWindow = mainWindow;
    this->mainWindow->addModule("AggDamageTakenGraph");
    ...
    QObject::connect(mainWindow->logParser, &t_parser::lineParsed
                     , this, t_eventHandler_damage_taken_agg::handleEvent);

    QObject::connect(this->chart, &QCustomPlot::selectionChangedByUser
                     , this, t_eventHandler_damage_taken_agg::graphSelectionChanged);
}

void t_eventHandler_damage_taken_agg::setupPlayField()
{
    this->playField = new QMdiSubWindow();
    this->playField->setMinimumSize(400, 200);
    this->playField->setWindowTitle("Damage Taken Aggregate");
...
void t_eventHandler_damage_taken_agg::handleEvent(t_logLine *log)
{
    QString data = "";
    ...
    if(match.hasMatch()){
        ...
        this->graphLines[target]->addData(offset, this->aggDamage[target]);
...
void t_eventHandler_damage_taken_agg::addGraph(QString entity)
{
...
void t_eventHandler_damage_taken_agg::graphSelectionChanged()
{
